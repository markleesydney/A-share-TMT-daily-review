...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'9/1 数据日 TMT 高位大分化：传媒 <b>{pct(media["pct_w"])}</b> 领涨（芒果超媒+20.02%两连板/掌阅科技+10%/中广天择+10%——首部AI长剧《后西游记》开播，数字媒体+8.94%），'
             f'计算机 <b>{pct(comp["pct_w"])}</b>（华信永道+29.96%/常山北明+9.98%/华胜天成+9.97%金融IT/软件），'
             f'通信 <b>{pct(comm["pct_w"])}</b>（光库科技-8.03%/锐捷网络-6.25%光模块续跌，恒宝股份/楚天龙数字货币涨停），'
             f'电子 <b>{pct(elec["pct_w"])}</b>（方邦股份-11.06%/中船特气-8.82%半导体材料杀跌，慧为智能+11.09%/波导股份+10.04%端侧逆势）。'
             f'9/2 收盘大盘普跌、双创领跌：沪指-0.97%报3941.39、创业板-2.39%报3312.24、科创50-1.82%，成交1.79万亿缩量2422亿创4月8日新低，超3900股下跌；军工装备+2.06%/美容护理+1.52%逆势，TMT全线调整（电子净流出363亿/通信-203亿/计算机-174亿）。'
             f'9/2 导火索：隔夜美股三连阴+费城半导体-2.13%+中东油价冲高+美联储加息预期升温（「油价+利率」双击），叠加中报季后资金「防御/低估值再平衡」，高久期TMT成长首当其冲——半导体/光模块/存储杀跌，军工/农业/银行承接避险。'
             f'中期主线（9月涨价函集中生效+国产替代规模兑现+华为MateXT2/苹果折叠屏密集催化）未变：Alpha派PaiPai指信息科技26Q2利润+46.7%、电子+134.8%，回调是「折现率与风险偏好的波动」而非产业景气否定。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块9/1全线杀跌：{top_l3.get("sub3","")}-1.796%相对抗跌（{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨，瑞芯微+4.63%/英集芯+5.18%逆势，AI SoC/电源管理端侧）；'
             f'模拟芯片设计-1.846%（5涨31跌）；'
             f'集成电路制造-2.266%（0涨7跌）；'
             f'半导体设备-2.856%（3涨24跌，芯源微-7.57%/超纯应材-6.44%高位回调）；'
             f'集成电路封测-3.445%、分立器件-3.503%（0涨18跌）；'
             f'{bot_l3.get("sub3","")}-5.372%领跌（{bot_l3.get("n",0)}只中{bot_l3.get("down",0)}跌，中船特气-8.82%/欧莱新材-6.51%）。'
             f'9/2 半导体延续调整：中证半导体指数12749.95-1.73%，乐鑫科技-5.01%/晶晨股份-4.21%/唯捷创芯-5.24%续杀，但盛景微+10%半导体涨停、恒运昌+5.49%/美芯晟+4.75%/国科微+4.04%逆势。'
             f'驱动：隔夜费城半导体-2.13%+存储/光通信美股全线调整+「油价+利率」双击，A股半导体/元件/电子化学品集体杀跌；9月"涨价函集中生效"（松下CCL最高+30%、村田MLCC10月对中小客户提价50%）vs"获利回吐"博弈。'
             f'国产替代规模兑现逻辑未变：华虹宏力21亿美元增资无锡三期12英寸特色工艺产线+长鑫中报亮眼印证景气，国信证券指半导体设备材料受存储扩产+国产化率提升带动。'
...
<b>解读：</b>9/1数据日拥挤度结构性剧变——传媒成交占全市场3.787%、60日分位100%（极度拥挤，创60日新高——AI长剧《后西游记》开播引爆短剧/数字内容，芒果超媒20cm两连板带动传媒成交激增），计算机6.311%分位83.3%（偏拥挤，自8/25的20.0%骤升——金融IT/软件开发逆势走强），通信7.356%分位25.0%（不拥挤，自8/25的73.3%大幅回落——光模块/光器件杀跌、资金撤离），电子22.542%分位1.7%（不拥挤，续创新低——半导体/元件全线杀跌，成交占比降至历史低位）。9/2收盘验证了"极度拥挤必兑现"的规律：传媒高开低走转跌（文化传媒净流出32.35亿）、计算机主题交易降温，而前期已出清至低位的电子/通信在「油价+利率」双击下延续调整（电子净流出363亿/通信-203亿），资金切向军工（+2.06%）/农业/银行/美容护理等低位防御。判断：9/1的"资金弃科技、买防御与内容"在9/2演化为"高位科技集体退潮+低位防御轮动加速"，本质是折现率抬升下的估值收缩而非趋势反转——9月涨价函集中生效+国产替代规模兑现（华虹宏力增资/长鑫中报）+华为/苹果折叠屏密集催化支撑中期主线，后续关键看电子/通信拥挤度出清后能否随「业绩兑现」企稳回升，以确认科技主线内部再平衡。</div></div></div>''')
