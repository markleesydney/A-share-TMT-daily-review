...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'TMT高位回调：8/13 仅通信逆势涨 <b>{pct(comm["pct_w"])}</b>（锐捷网络+13.5%/星网锐捷/共进股份涨停，'
             f'中际旭创17.47亿入股中石科技卡位散热+对印度光纤反倾销），电子 <b>{pct(elec["pct_w"])}</b>（杰理科技IPO次日-12.5%、'
             f'风华高科-6.75%、半导体材料-3.74%领跌）、传媒 <b>{pct(media["pct_w"])}</b>（北京文化4连板后回落）、'
             f'计算机 <b>{pct(comp["pct_w"])}</b>（紫光股份+7%/浪潮信息+5.9%/城地香江3连板）。'
             f'8/14早盘沪指冲高回落-0.21%、超3400只下跌，存储芯片受闪迪投资者日"炸裂"指引(毛利率80%/FCF利润率50%)映射大涨'
             f'（联芸+7%/普冉+4.3%/兆易创新+3%），中石科技被入股一字涨停。'
             f'核心主线：存储周期重估 + 算力链(DeepSeek V4 Pro涨价/中芯华虹Q2超预期) + 光模块散热卡位。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块8/13普跌回调：分立器件逆势领涨（{top_l3.get("sub3","")}+{pct(top_l3.get("pct",0))}，{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨），'
             f'集成电路制造+0.57%（7只中2涨）；其余全线走弱——半导体材料-3.74%（{bot_l3.get("n",0)}只仅{bot_l3.get("up",0)}涨）、半导体设备-2.27%、'
             f'数字芯片设计-1.79%、封测-1.19%、模拟芯片设计-0.92%。'
             f'驱动：连续暴涨后高位获利了结，PCB/CCL与存储前期强势股（风华高科/胜宏/生益/景旺）集体承压。'
             f'8/14早盘存储芯片受闪迪投资者日超预期指引映射强势反弹（联芸+7%/普冉+4.3%/兆易创新+3%），中芯国际/华虹宏力Q2业绩超预期+Q3乐观指引，'
             f'DRAM受HBM产能挤占紧张格局延续至2027年。'
...
<b>解读：</b>电子成交占全市场27.60%但60日分位仅5%（占比虽高但处低位，8/13回调缩量），通信8.37%分位60%中性；计算机6.98%分位93.3%、传媒2.84%分位96.7%双双处于极度拥挤（换手活跃但绝对占比低，需警惕连板高标获利了结）。资金面8/14早盘通信净流入超53亿居前，存储芯片/CPO/电子化学品领涨，计算机净流出明显、传媒持续流出。资金从高位AI应用/软件向存储+算力硬件+光模块散热方向迁移。</div></div></div>''')
