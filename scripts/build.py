...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'9/2 数据日 TMT 全线调整：传媒 <b>{pct(media["pct_w"])}</b> 领跌（中文在线-7.17%/博纳影业-6.65%——AI长剧概念高开低走回吐，但大晟文化/龙版传媒/欢瑞世纪逆势涨停），'
             f'电子 <b>{pct(elec["pct_w"])}</b>（华亚智能-7.81%/中石科技-6.89%——半导体/元件杀跌，消费电子+0.743%唯一逆势），'
             f'通信 <b>{pct(comm["pct_w"])}</b>（中际旭创-4.29%/新易盛-4.00%光模块续跌，恒宝股份/楚天龙数字货币逆势），'
             f'计算机 <b>{pct(comp["pct_w"])}</b>（和仁科技-7.51%/麒麟信安-5.60%，竞业达4连板/路桥信息+21.64%逆势）。'
             f'9/2 收盘大盘普跌、双创领跌：沪指-0.97%报3941.39、创业板-2.39%报3312.24、科创50-1.82%，成交约1.79万亿缩量2422亿，超3900股下跌；导火索为隔夜美股三连阴+费城半导体-2.13%+中东油价冲高+美联储加息预期升温（「油价+利率」双击），高久期TMT成长首当其冲。'
             f'9/3 收盘缩量震荡、赚指数不赚钱：沪指+0.02%报3942.09、深成指+0.10%、创业板+0.01%，成交1.76万亿缩量323亿、超3500股下跌；午后美商务部长放风「考虑对进口半导体加征新关税」引发全球股市跳水（A股半导体跌超1%、但存储走出国产替代独立行情），液冷服务器/航运/培育钻石走强 vs 高位人气股杀跌，主力净流出电子50.17亿居首、中际旭创遭净卖11.68亿。'
             f'中期主线未变：Alpha派PaiPai（Think+web-search 47引用）指TMT「基本面强现实、交易面弱预期」——通信（运营商/光通信）相对抗跌>电子>计算机/传媒，光通信「双雄」中报合计净利超210亿、博通AI收入指引FY28 2300亿美元、OCS光交换成新热点，回调是「折现率与风险偏好的波动」而非产业景气否定。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块9/2全线杀跌：{top_l3.get("sub3","")}-1.042%相对抗跌（{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨，英集芯等端侧电源管理逆势）；'
             f'分立器件-1.669%（2涨16跌，士兰微-4.17%）；'
             f'数字芯片设计-1.703%（6涨48跌，乐鑫科技-5.01%/晶晨股份-4.21%）；'
             f'半导体设备-1.794%（5涨22跌，长川科技-4.11%）；'
             f'半导体材料-1.86%（3涨25跌）；'
             f'集成电路制造-2.213%（0涨7跌）；'
             f'{bot_l3.get("sub3","")}-2.408%领跌（{bot_l3.get("n",0)}只中{bot_l3.get("down",0)}跌）。'
             f'9/3 收盘半导体受「关税」消息午后走弱跌超1%：美商务部长称考虑对进口半导体加征新一轮关税（微导纳米/汇成股份跌超7%、矽电股份/盛科通信跌超6%），但存储链走出「外盘杀风险、内盘炒替代」独立行情（长鑫科技一度涨近3%）；第三代半导体逆势爆发（三安光电+9.98%放量涨停、净流入11.45亿），液冷散热（思泉新材20cm涨停）活跃。'
             f'驱动：9/2隔夜费城半导体-2.13%+「油价+利率」双击，A股半导体/元件/电子化学品集体杀跌；9/3关税利空+资金回避高位→半导体承压、主力净流出电子50.17亿居首，但国产替代逻辑反而强化。'
             f'9月「涨价函集中生效」（松下CCL最高+30%、村田MLCC10月对中小客户提价50%）+国产替代规模兑现（华虹宏力增资无锡三期/长鑫中报亮眼/中微公司纳入富时A50）+高盛上调2026全球晶圆厂设备支出增速至45%（1500亿美元）支撑中期景气。'
...
<b>解读：</b>9/2数据日拥挤度——电子21.265%分位1.7%（不拥挤，续创新低——半导体/元件杀跌、成交占比降至历史低位），通信7.549%分位28.3%（不拥挤，自9/1的25.0%小幅回升），计算机5.393%分位50.0%（中性，自9/1的83.3%骤降——金融IT/软件主题交易降温），传媒3.179%分位95.0%（极度拥挤，自9/1的100%微降——AI长剧概念高潮后仍处高位）。9/2收盘验证「极度拥挤必兑现」：传媒高开低走转跌（中文在线-7.17%/博纳影业-6.65%），电子/通信/计算机延续调整。9/3收盘科技拥挤度进一步出清、资金高低切换：主力净流出电子50.17亿/通信/半导体，转而进攻第三代半导体（+24.7亿）、液冷、航运、培育钻石等低位题材与贵金属/保险防御；高位人气股（传智教育等）集体跌停。判断：9/2的「高位科技集体退潮」本质是折现率抬升下的估值收缩而非趋势反转，拥挤度出清+9月涨价函集中生效+国产替代规模兑现（华虹宏力增资/长鑫中报/中微公司纳入富时A50）+华为/苹果折叠屏密集催化共振，后续关键看电子/通信能否随「业绩兑现」企稳回升，以确认科技主线内部再平衡；短期通信（运营商/光通信）相对抗跌、传媒拥挤度仍高需防高位兑现。</div></div></div>''')
