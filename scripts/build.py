...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'TMT全线放量大涨：电子 <b>{pct(elec["pct_w"])}</b>（长鑫科技+12%市值破4.13万亿、通富微电涨停吸金22.81亿居A股第一、'
             f'中石科技20cm 2连板/苏州天脉20cm/金禄电子+19.99%），通信 <b>{pct(comm["pct_w"])}</b>（太辰光20cm涨停/仕佳光子+13.67%/'
             f'共进股份3连板/中瓷电子涨停，CPO概念+4.22%），计算机 <b>{pct(comp["pct_w"])}</b>（概伦电子+16.38%/鸿博股份涨停/广立微+8.3%，'
             f'朗新科技-10.99%领跌），传媒 <b>{pct(media["pct_w"])}</b>（北京文化4连板后跌停/儒意电影-8.34%/中广天择涨停，唯一收跌）。'
             f'大盘放量上攻：沪指+1.41%报3982.65点、创业板指+3.14%破3700点、成交2.40万亿放量2459亿、超4300只上涨。'
             f'核心主线：摩根大通\'Q2半导体财报看涨、涨价从存储扩散至设备材料\'+SK海力士崔泰源强调存储需求爆发+三星/SK海力士五年期长协锁量 + '
             f'英伟达Spectrum-X硅光交换机全面量产+中际旭创17.47亿入股中石科技卡位散热 + '
             f'7月高技术制造业+16.9%（集成电路产量+20.7%）。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块8/17全线爆发：集成电路封测领涨（{top_l3.get("sub3","")}+{pct(top_l3.get("pct",0))}，{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨）、'
             f'数字芯片设计+7.22%、分立器件+5.49%、半导体材料+5.25%、半导体设备+5.03%、集成电路制造+4.72%、模拟芯片设计+3.02%，'
             f'七大三级细分行业几乎全数普涨（{bot_l3.get("sub3","")}相对最弱仅+{pct(bot_l3.get("pct",0))}）。'
             f'驱动：长鑫科技+12%创新高市值破4.13万亿（成交335亿/主力净流入超30亿）领涨，通富微电涨停吸金22.81亿居A股第一，'
             f'摩根大通\'Q2全球半导体财报看涨、涨价从存储延伸至设备与材料\'+三星/SK海力士五年期长协锁量+Omdia上调2026半导体市场+94.1%，'
             f'先进封装/半导体设备/存储芯片方向集体走强。'
...
<b>解读：</b>电子成交占全市场30.54%、60日分位41.7%（放量上行但分位仍处中性，8/17半导体/算力硬件资金大幅回流），通信9.96%分位95.0%达极度拥挤（CPO/光通信连续走强后换手活跃，需警惕高标获利了结）；计算机5.71%分位68.3%中性、传媒2.03%分位73.3%偏拥挤（绝对占比低但影视题材前期已积累涨幅）。资金面8/17电子净流入179.72亿/通信+64.8亿合计占全市场净流入近75%，计算机净流出21.45亿/传媒净流出10.91亿。资金主线高度集中于存储+半导体设备/封测+CPO/光模块散热方向，软件/影视应用端遭分流。</div></div></div>''')
