...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'8/24 数据日 TMT 全线重挫、通信领跌：电子 <b>{pct(elec["pct_w"])}</b>（耐科装备+16.28%/有研硅+9.25%硅晶圆涨价，普冉股份-11.36%/中英科技-12.02%），'
             f'通信 <b>{pct(comm["pct_w"])}</b>（长飞光纤+4.29%/楚天龙+10.01%涨停，中际旭创-7.72%/天孚通信-8.63%/新易盛-6.79% CPO 杀跌），'
             f'计算机 <b>{pct(comp["pct_w"])}</b>（行云科技+4.95%算力租赁，旋极信息-10.14%），'
             f'传媒 <b>{pct(media["pct_w"])}</b>（天威视讯+5.98%，昆仑万维-7.49%/芒果超媒-6.99%）。'
             f'8/25 收盘探底回升、TMT 反弹分化：沪指+0.19%报3889.44 翻红、创业板-1.00%续跌、成交缩至 1.83 万亿创下半年新低、超 4200 股上涨；'
             f'传媒+1.64%/计算机+1.40%/CPO+1.02% 领涨，半导体续弱-0.31%，液冷（英维克涨停）与光纤光缆（亨通+6.90%/中天+4.18%/长飞+4.04%）成资金新焦点，光模块"易中天"续跌（中际旭创-2.78%/新易盛-2.70%）。'
             f'8/24 杀跌导火索：SK海力士《自然·电子学》CPO 论文+英伟达 CPO 量产→"易中天"单日蒸发超 1500 亿。'
             f'中期主线（功率半导体第三轮涨价+硅晶圆三年首涨+长江存储 IPO+HBM 国产化）未变，光通信中报集中翻倍（新易盛+90.98%/中际旭创+241.70%）夯实高景气逻辑。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块8/24普跌、仅材料逆势：{top_l3.get("sub3","")}+0.324%唯一上涨（{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨，有研硅+9.25%/珂玛科技+4.71%/恒坤新材+3.85%，硅晶圆三年来首次大幅涨价）；'
             f'半导体设备-0.688%相对抗跌（耐科装备+16.28%领涨全电子/京仪装备+3.55%，高盛上调2026-2028全球半导体设备支出预期）；'
             f'分立器件-2.416%、集成电路制造-2.501%、模拟芯片设计-3.961%、集成电路封测-4.108%、'
             f'{bot_l3.get("sub3","")}-4.775%领跌（{bot_l3.get("n",0)}只中{bot_l3.get("down",0)}跌，普冉股份-11.36%/新相微-7.84%）。'
             f'驱动：8/24 存储链在长江存储IPO+存储涨价利好兑现后高开低走（普冉股份领跌），模拟/封测/数字设计随CPO杀跌与风险偏好回落普跌；'
             f'唯半导体材料/设备逆势，反映"涨价+国产替代+资本开支"主线在杀跌中仍获资金认可。'
             f'8/25 收盘半导体续弱（中证半导体-0.31%）、资金净流出21.16亿，设备高位分化回调（华峰测控/拓荆科技/微导纳米大跌，兆易创新+3.63%逆势），但功率半导体（*ST闻泰涨停）、散热（飞荣达+6.22%）等"卖铲人"方向获资金回流。'
             f'中期逻辑强化：蓝宝书H7功率半导体第三轮涨价（新洁能/华润微9月提价、毛利率逐季加速）+硅晶圆三年首涨+长江存储IPO+HBM国产化，寒武纪目标价上调至1841元。'
...
<b>解读：</b>8/24数据日拥挤度整体回落：通信成交占全市场9.036%、60日分位81.7%（偏拥挤，自8/21的86.7%小幅回落但仍处高位——8/24通信-4.176%领跌、成交1826亿，CPO杀跌后资金分歧加大，高位拥挤仍需警惕），电子26.443%分位6.7%（不拥挤，半导体材料/设备逆势、但整体普跌），计算机4.247%分位13.3%（不拥挤，信创/算力资金流出），传媒1.366%分位15.0%（不拥挤，避险资金撤离）。8/24的CPO/算力硬件暴跌，正是8/21通信拥挤度骤升至86.7%后的高位兑现——SK海力士CPO论文+英伟达CPO量产触发，资金在通信/半导体链集中度过高后集中获利了结（电子主力净流出355.9亿、通信设备139.6亿），黄金/煤炭/银行/种业等低位防守方向承接流出资金。8/25收盘资金高低切换延续：通信板块主力净流入27.21亿居行业第三、但光模块龙头"易中天"续跌（中际旭创主力净流出21.29亿/新易盛-6.36亿），资金从"光模块龙头"切向"液冷/散热+光纤光缆"（英维克+15.96亿/亨通光电+18.02亿/中天科技+9.21亿/长飞光纤+7.32亿），通信拥挤度81.7%分位（偏拥挤）仍需警惕，但成交额已缩至1.83万亿下半年新低。判断8/24是"高位拥挤兑现"而非趋势终结，光通信中报集中翻倍（新易盛+90.98%/中际旭创+241.70%）夯实景气；后续关键看通信拥挤度能否温和回落至40%分位下方、成交额能否重回1.9万亿以上，以确认算力主线能否重启。</div></div></div>''')
