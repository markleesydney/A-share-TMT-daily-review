...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'算力硬件/CPO主线回归、功率半导体涨价接棒：电子 <b>{pct(elec["pct_w"])}</b>（8/20模拟芯片设计强：思瑞浦+15.31%/康希通信+11.17%，'
             f'半导体材料弱：沪硅产业-8.43%/西安奕材-8.16%），通信 <b>{pct(comm["pct_w"])}</b>（太辰光+18.38%/长盈通+18.41%/联特科技+13.45%，光迅科技-2.66%），'
             f'计算机 <b>{pct(comp["pct_w"])}</b>（罗普特+11.02%/国新健康+7.3%，合合信息-13.15%/高伟达-12.25%），'
             f'传媒 <b>{pct(media["pct_w"])}</b>（昆仑万维+5.95%/百纳千成+6.21%，中信出版-9.17%/北京文化-5.28%）。'
             f'8/21收盘大盘震荡分化：沪指+0.04%、创业板指+1.43%、科创50+0.04%，成交1.88万亿缩量2001亿（年内第9次不足2万亿），超2800股下跌。'
             f'8/21算力硬件/CPO领涨：星网锐捷/瑞斯康达/通鼎互联涨停，芯片概念主力净流入162.5亿、CPO 155.7亿、5G 150.7亿。'
             f'核心主线：阿里Capex 677亿超预期(+75%/环比+152%)+云收入+45%九连加速+功率半导体第三轮涨价（台厂10月落地涨10-15%）+SK海力士40万亿韩元回购，'
             f'光模块/算力硬件由"超跌反弹"走向"主线确认"，但量能缩至1.88万亿、持续性待观察。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块8/20内部分化逆转：模拟芯片设计领涨、材料回调——模拟芯片设计+5.984%（{top_l3.get("sub3","")}居首，{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨）、'
             f'半导体设备+0.691%、分立器件+0.087%；材料与制造回调——集成电路封测-0.173%、数字芯片设计-0.242%、'
             f'集成电路制造-1.402%、半导体材料-3.814%（{bot_l3.get("sub3","")}最弱，{bot_l3.get("n",0)}只中{bot_l3.get("down",0)}跌）。'
             f'驱动：模拟芯片设计思瑞浦+15.31%/康希通信+11.17%/敏芯股份+10.45%领涨（思瑞浦Q2收入环比+28.65%、光模块收入超1.8亿成通信板块第一大细分业务，'
             f'8/21功率半导体接棒：士兰微/中瓷电子/旭光电子涨停）；半导体材料沪硅产业-8.43%/西安奕材-8.16%/盛合晶微-9.23%领跌（前期涨价潮大涨后获利回吐）。'
             f'中证半导体指数8/20-0.62%，8/21功率半导体第三轮涨价（台厂10月落地涨10-15%）接力，反映"涨价主线从材料/模拟切向功率器件"，上游材料高位拥挤度释放、功率/模拟环节接棒。'
...
<b>解读：</b>8/19大跌后资金撤离 TMT，四行业拥挤度全面回落（均为8/20数据日口径）：电子成交占全市场26.284%、60日分位骤降至3.3%（极度不拥挤——前期31%高位回落至近两月最低，半导体材料/设备获利回吐），通信7.836%分位36.7%中性，计算机4.468%分位23.3%不拥挤（前期71.7%高位大幅降温），传媒1.637%分位43.3%中性。8/21收盘算力硬件/CPO主线回归（芯片概念净流入162.5亿/CPO 155.7亿），资金重新定向加码通信链，但全市场成交1.88万亿缩量2001亿（年内第9次不足2万亿），量能未同步放大，反弹持续性仍需观察，判断"超跌反弹"还是"趋势反转"的关键仍是成交额能否重回2万亿上方。</div></div></div>''')
