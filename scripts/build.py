...
占全市场 {num(c["share_now"],2)}%（60日 {num(c["share_pctile_60d"],1)} 分位）</div></div>''')
    ...
    p.append(f'<div class="note"><b>一句话概括：</b>'
             f'8/21 数据日 TMT 分化、通信领涨：电子 <b>{pct(elec["pct_w"])}</b>（士兰微+9.99%/扬杰科技+7.01%功率涨价，锦富技术+12.5%消费电子，上海新阳-7.38%/中石科技-7.25%回吐），'
             f'通信 <b>{pct(comm["pct_w"])}</b>（星网锐捷/通鼎互联/瑞斯康达/中瓷电子涨停，新易盛+6.76%，数据港-9.5%），'
             f'计算机 <b>{pct(comp["pct_w"])}</b>（概伦电子+10.8%/四方精创+5.26%，诚迈科技-9.78%/中国软件-5.7%），'
             f'传媒 <b>{pct(media["pct_w"])}</b>（*ST三六五+6.7%/中广天择+4.64%，昆仑万维-2.73%/百纳千成-4.68%）。'
             f'但 8/24 收盘 TMT 全线重挫、利好兑现杀跌：沪指-0.59%/深成-2.13%/创业板指-3.21%/科创50-3.10%，成交 2.01 万亿放量 1282 亿、仅 26% 个股上涨，'
             f'通信设备-5.21%/半导体-3.13%领跌，电子主力净流出 355.9 亿。三重导火索：①SK海力士《自然·电子学》CPO 论文+英伟达 CPO 量产→"易中天"单日蒸发超 1500 亿（天孚通信-10.48%/中际旭创-7.72%/新易盛-6.79%）；'
             f'②FCC 光模块政策黑天鹅反向解读；③阿里 800 亿港元配售拖累港股科技（恒生科技-3.84%）。'
             f'煤炭/贵金属/银行/保险逆势（焦炭首轮提涨+黄金突破 4650 美元），资金高低切换至防守。'
    ...
    p.append(f'<div class="dyn"><div class="d">半导体板块8/21内部分化：功率半导体（分立器件+3.047%）领涨、材料/设备回调——分立器件+3.047%（{top_l3.get("sub3","")}居首，{top_l3.get("n",0)}只中{top_l3.get("up",0)}涨）、'
             f'数字芯片设计+0.973%、集成电路制造+0.609%、模拟芯片设计+0.359%；半导体设备-0.065%、集成电路封测-0.125%、'
             f'半导体材料-0.815%（{bot_l3.get("sub3","")}最弱，{bot_l3.get("n",0)}只中{bot_l3.get("down",0)}跌）。'
             f'驱动：功率半导体涨价主线延续——士兰微+9.99%（成交27亿）/旭光电子+10%/扬杰科技+7.01%/华润微+4.66%领涨（半导体新一轮涨价潮：存储/MCU上调、TI/意法多轮提价10-30%、交付周期拉长至30周、成熟制程缺货最突出）；'
             f'存储链受长江存储IPO获受理刺激（东芯股份+5.58%/德明利+4.28%）；半导体材料有研新材-4.27%/有研硅-1.73%领跌（前期硅片/材料涨价潮大涨后获利回吐）。'
             f'8/24 收盘半导体板块高开低走大幅回调（申万半导体-3.13%）：普冉股份跌超11%/兆易创新-6.7%/寒武纪-6.37%/中芯国际-3.38%，主力净流出 180.6 亿居市场前列，'
             f'长江存储IPO+硅晶圆涨价超10%等利好兑现后获利了结；但 Alpha派蓝宝书提示中期三重逻辑未变——长江存储耗材采购利好零部件（富创精密/正帆科技/新莱应材/珂玛科技）、HBM国产化替代（联芸科技/江波龙/澜起科技）、意法年内第三次涨价。'
...
<b>解读：</b>8/21数据日拥挤度显著分化：通信成交占全市场9.264%、60日分位骤升至86.7%（偏拥挤，自8/20的36.7%大幅回升——8/21通信+2.503%领涨、成交1752.9亿，资金重新涌入通信链，光模块高拥挤需警惕分歧），电子25.824%分位1.7%（极度不拥挤，续创新低，半导体材料/设备持续回吐），计算机4.317%分位16.7%（不拥挤，信创资金流出），传媒1.395%分位16.7%（不拥挤，避险资金撤离）。8/24收盘的CPO/算力硬件暴跌，正是8/21通信拥挤度骤升至86.7%后的高位兑现——SK海力士CPO论文+英伟达CPO量产+阿里配售三重利空触发，资金在通信/半导体链集中度过高后集中获利了结（电子主力净流出355.9亿、通信设备139.6亿），黄金/煤炭/银行/种业等低位防守方向承接流出资金。判断后续关键：通信拥挤度能否从高位温和回落、成交额能否重新放量，决定本轮算力主线是"高位震荡"还是"趋势延续"；若拥挤度快速降温至40%分位下方且成交缩量企稳，则更可能是技术性调整而非趋势终结。</div></div></div>''')
